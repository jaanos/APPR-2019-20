...
#HGN igralci po sezonah (homegrown) - po brexitu so tudi Irci HGN, zato so vkljuèeni
...
napredna_analiza_graf2 <- ggplot(data=tabela_napredna_analiza2, aes(x=sezone_Klopp$Sezona, y=vektor_napredna_analiza2,group=1)) +
  geom_point() + geom_line() + geom_smooth(method="lm", formula=vektor_napredna_analiza2 ~ c(1:5) + I(c(1:5)^2))+
  expand_limits(x = 7)
